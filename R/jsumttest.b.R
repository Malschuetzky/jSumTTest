jSumTTestClass <- if (requireNamespace('jmvcore', quietly=TRUE)) R6::R6Class(
    ...
    private = list(
        .run = function() {
          ...
          CIdiff_Wd <- CIdiff_w/100 # decimal value
          CIdiff_w_2s <- CIdiff_w+((100-CIdiff_w)/2) # two-tailed value
          CIdiff_Wd_2s <- CIdiff_w_2s/100 # decimal value two-tailed
          ...
          # local variables ini
          table1 <- self$results$ttesttable
          table2 <- self$results$desctable
          ...
          ## calculate Convidence Interval for means (Eid et al., 2017, F 8.16)
          # calculate z-value of given CI width
          z_CI_value <- qnorm(CI_M_w/100)
          # calculate CI boarders
          ...
          # calculate Convidence Interval for Cohen's d @ Student's t-Test (Revelle, 2023)
          dCI_Welch <- psych::d.ci(d_Welch, n1=n1, n2=n2, alpha=CI_d) # x[1]=lower value, x[2]=d, x[3]=upper value
          ...
          if (hypo_tail == 'notequal') {
            ...
            p_Stud <- 2*pt(q=abs(t_Stud), df=df_Stud, lower.tail=FALSE)
          } else if (hypo_tail == 'onegreater') {
            # one-tailed test (M1 > M2)
          ...
          # calculate Convidence Interval for Cohen's d @ Student's t-Test (Revelle, 2023)
          dCI_Stud <- psych::d.ci(d_Stud, n1=n1, n2=n2, alpha=CI_d)  # x[1]=lower value, x[2]=d, x[3]=upper value          
          CI_d_S_low <- dCI_Stud[1]
          CI_d_S_upp <- dCI_Stud[3]          
          
          ## calculate Convidence Interval (Eid et al., 2017, eq. F 11.14a - F 11.14c)
          if (hypo_tail == 'notequal') {
            # t-critical readout for CI, (M1 != M2), (Eid et al., 2017, eq. F 11.14a)
